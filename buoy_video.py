...
lower_red = np.array([160,45,45])
upper_red = np.array([179,255,255])


cap = cv2.VideoCapture('output.mp4')

def findGreenBuoy(frame):
	...
	if(len(cnts) > 0):
		...
		if(contours_count > 0):
			...
			cv2.putText(frame,'GREEN',(center[0],center[1]), 1, 2,(255,255,255), 1,cv2.LINE_AA)
...
def findRedBuoy(frame):
	...
	if(len(cnts) > 0):
		...
		if(contours_count > 0):
			...
			center = (contours_x,contours_y)
			cv2.circle(frame,center,10,(255,0,255),-1)
			cv2.putText(frame,'RED',(center[0],center[1]), 1, 2,(255,255,255), 1,cv2.LINE_AA)
		approx = cv2.approxPolyDP(c, 0.1 * peri, True)
		cv2.drawContours(frame, [approx], -1, (0, 255, 0), 3)
